...
public:	
	...
	static int floatToIntInRound(float f){
		return Round(f);
	}
	...
	static Point* getRotatedPoint(int x, int y, int centerX, int centerY, int alpha){
		int finalX = ROUND (  (float)(x-centerX) * COS(alpha) - (float)(y - centerY)*SIN(alpha) + (float)centerX  ) ;
		int finalY = ROUND (  (float)(x-centerX) * SIN(alpha) + (float)(y - centerY)*COS(alpha) + (float)centerY  ) ;
